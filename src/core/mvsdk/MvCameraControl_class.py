...
def _load_mvs_sdk():
    ...
    if system == "Windows":
        ...
        dll_dir = next((d for d in candidate_dirs if os.path.exists(os.path.join(d, dll_name))), None)
        if dll_dir:
            # Python 3.8+ 不再使用 PATH 解析 DLL 依赖，必须显式添加
        ...
        if "winmode" in ctypes.WinDLL.__init__.__code__.co_varnames:
            return ctypes.WinDLL(full, winmode=0)
        return ctypes.WinDLL(full)

    if system == "Darwin":
        return ctypes.cdll.LoadLibrary("/usr/local/lib/libMvCameraControl.dylib")

    arch = platform.machine()
    runenv = os.getenv("MVCAM_COMMON_RUNENV", "/opt/MVS/lib")
    subdir = "64" if is_64bit else "32"
    if arch == "aarch64":
        subdir = "aarch64"
    elif arch.startswith("arm"):
        subdir = "armhf"
    return ctypes.cdll.LoadLibrary(f"{runenv}/{subdir}/libMvCameraControl.so")


MvCamCtrldll = _load_mvs_sdk()
